...
class TestQuotesBooksImpl(unittest.TestCase):
  ...
  def test_children(self):
    ...
    self.assertEquals(len(quotes1), 1)
    self.assertEquals(speakers[0].pretty(), "Alice Roosevelt Longworth")
    quotes2 = speakers[1].children()
    self.assertEquals(len(quotes2), 7)
    self.assertEquals(speakers[1].pretty(), "Mark Twain")
