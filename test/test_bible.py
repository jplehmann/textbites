...
class TestBibleBooksImplWithBible(unittest.TestCase):
  ...
  def test_first_and_last_books(self):
    ...
    self.assertEqual(len(bible.children()), 66)
    self.assertEqual(bible.children()[0].title, "Genesis")
    self.assertEqual(bible.children()[-1].title, "Revelation")

  def test_parse_reference(self):
    # Line
    ref = self.res.reference("jn 3:16")
    self.assertEqual(ref.pretty(), "John 3:16")
    # LineRange
    ref = self.res.reference("1 jn 4:2-5")
    self.assertEqual(ref.pretty(), "1 John 4:2-5")
    # Chapter
    ref = self.res.reference("jn 3")
    self.assertEqual(ref.pretty(), "John 3")
    # Chapter
    ref = self.res.reference("john 3")
    self.assertEqual(ref.pretty(), "John 3")
    # ChapterRange
    ref = self.res.reference("jn 3-4")
    self.assertEqual(ref.pretty(), "John 3-4")
    # Book
    ref = self.res.reference("1 jn")
    self.assertEqual(ref.pretty(), "1 John")

  def test_search_chapter(self):
    ref = self.res.reference("jn 3")
    hits = ref.search("love")
    hit_refs = [h.pretty() for h in hits]
    self.assertEqual(len(hit_refs), 3)
  ...
  def test_search_book(self):
    ...
    hit_refs = [h.pretty() for h in hits]
    self.assertEqual(len(hit_refs), 84)
  ...
  def test_previous_edge(self):
    ...
    self.assertEqual(ref.previous(), None)
    ref = self.res.reference("jn 3:2")
    self.assertEqual(ref.previous().pretty(), "John 3:1")

  def test_next(self):
    ref = self.res.reference("jn 3:16")
    self.assertEqual(ref.next().pretty(), "John 3:17")

  def test_next_edge(self):
    ref = self.res.reference("jn 3:36")
    self.assertEqual(next(ref), None)
    ref = self.res.reference("jn 3:35")
    self.assertEqual(ref.next().pretty(), "John 3:36")

  def test_parent_line(self):
    ref = self.res.reference("jn 3:1")
    self.assertEqual(ref.parent().pretty(), "John 3")
  ...
  def test_parent_chapter(self):
    ref = self.res.reference("jn 3")
    self.assertEqual(ref.parent().pretty(), "John")
  ...
  def test_path(self):
    ref = self.res.reference("jn 3:1-3")
    self.assertEqual(ref.path(), "John 3:1-3")
  ...
  def test_normalize_book_name(self):
    ...
    self.assertEqual(normalize_book_name("John"), "John")
    self.assertEqual(normalize_book_name("jn"), "John")
    self.assertEqual(normalize_book_name("jN"), "John")
    self.assertEqual(normalize_book_name("1jn"), "1 John")
    self.assertEqual(normalize_book_name("1jn"), "1 John")
    self.assertEqual(normalize_book_name("mk"), "Mark")
